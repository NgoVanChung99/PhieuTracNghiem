...
def crop_image(img):
    ...
    #image_tmp = cv2.resize(img_canny, (1024,768))
    #cv2.namedWindow("Apply canny edge",cv2.WINDOW_FULLSCREEN)
    cv2.imshow("Apply canny edge",img_canny)
    #cv2.waitKey()
...
def process_ans_blocks(ans_blocks):
    ...
    for ans_block in ans_blocks:
        ...
        cv2.imshow("Ans_block_img "+ str(ans_block) , ans_block_img)
        #cv2.waitKey()
        ...
        for i in range(6):
            ...
            cv2.imshow("Box_img " + str(i) , box_img)
            #cv2.waitKey()
            # loop over each line in a box
            for j in range(5):
                list_answers.append(box_img[j * offset2:(j + 1) * offset2, :])
                # cv2.imshow("Box_img_answers "+str(j),box_img[j * offset2:(j + 1) * offset2, :])
                #cv2.waitKey()
...
def process_list_ans(list_answers):
    ...
    for answer_img in list_answers:
        for i in range(4):
            ...
            #cv2.imshow("List_choices " + str(answer_img)+str(i) , bubble_choice)
...
if __name__ == '__main__':
    ...
    print(answers)
    print(DapAn)
    ResultLen = len(DapAn)-1      #so dap an file excel
    AnswerLen = len(answers)-1    #so dap an ghi nhan tren phieu
    sumIn =float(10/ResultLen) 
    count =0
    if AnswerLen <= ResultLen :
        for index in answers :
            tmp=''.join(map(str, answers[index]))
            print(index)
            print(tmp)
            print(DapAn[index][1])
            if(DapAn[index][1]==tmp):
                print("true")
                count+=1
            else : print("false")
